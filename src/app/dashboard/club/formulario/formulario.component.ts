...
export class FormularioComponent {
   numero:any=/[0-9]+/;
    ...
    constructor(public dialogRef:MatDialogRef<FormularioComponent>, @ Inject (MAT_DIALOG_DATA) public data:any,private clubServicio:ClubService){
      console.log(data)
      this.texto=data.texto
      this.id?.setValue(data.club.id)
      this.nombre?.setValue(data.club.nombre)
      this.apellido?.setValue(data.club.apellido)
      this.username?.setValue(data.club.username)
      this.password?.setValue(data.club.password)
      this.rol?.setValue(data.club.rol)
      if(data.texto=="Editar Usuario"){
        this.password?.clearValidators()
        this.imagen?.clearValidators()
        if(data.club.imagen!="")
        this.previsualizacion=this.base+'club/imagen/'+data.club.imagen
      }
    }
    ...
    cargarImagen(event:any):void{
      ...
      this.clubServicio.subirImagen(file,this.name).subscribe(data=>{
      })
    }
